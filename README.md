...
- `Theme.Apply()` 가 팔레트 사전의 **값을 대상 테마 값으로 교체**하고,
- XAML 은 팔레트 브러시를 전부 **`{DynamicResource}`** 로 참조한다(값이 바뀌면 WPF 가 재평가).
  코드는 `Theme.Brush()/Color()` 로 쓰는 순간 조회한다.

시행착오 기록: 처음에는 "브러시 인스턴스의 `Color` 만 제자리 변경 + `StaticResource`" 로 설계했는데,
측정해 보니 그 경로는 **한 번도 실행된 적이 없었다** — WPF 가 BAML 사전의 Freezable 을 자동
Freeze 하고, 사전에 새로 넣는 값도 즉시 다시 Freeze 한다(38/38 확인). 그래서 실행 중 전환이
쓰는-순간-조회 경로에만 먹고 `StaticResource` 참조(메뉴·상태바)에는 먹지 않았다.
`DynamicResource` 전환으로 해결했고, 회귀는 규약 테스트가 막는다(팔레트 브러시를
`StaticResource` 로 참조하면 빌드가 깨진다). 사전 통째 교체는 여전히 안 된다 — 값 교체여야 한다.
